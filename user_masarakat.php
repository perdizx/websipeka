...
<input type="text" name="cr" placeholder="Cari User Masyarakat" style="width:20%; padding:8px; margin-top:5px; margin-bottom:5px" class="form_input" />
...
$cari = $_POST ['cr'];
if ($cari !=''){
$query = mysqli_query ($conn, "SELECT * FROM masarakat WHERE nama LIKE '%".$cari."%'
	OR nik LIKE '%".$cari."%' OR alamat LIKE '%".$cari."%'");}else{
	$query = mysqli_query ($conn, "SELECT * FROM masarakat LIMIT $posisi,$batas");
}
...
<input type="submit" class="btn btn-success" style="padding:8px" name="cari" value="Cek Validasi" /></td>
</form>
</td>
</tr>
<?php }}else{
...
<?php
	$query2		= mysqli_query($conn, "SELECT * FROM masarakat");
...
<li class="page-item"><a class="page-link" style="color:blue">Halaman</a></li>
	...
	if ($i != $halaman){
		echo "<li class='page-item'><a class='page-link' href=\"user_masarakat.php?halaman=$i\">$i</a></li>";
	}
	else{
